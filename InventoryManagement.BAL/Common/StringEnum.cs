...
namespace InventoryManagement.BAL.Common
{
    ...
    /// <summary>
    /// Helper class for working with 'extended' enums using <see cref="DisplayValueAttribute"/> attributes.
    /// </summary>
    public class StringEnum
    {
        ...
        public Array GetStringValues()
        {
            ...
            foreach (FieldInfo fi in _enumType.GetFields())
            {
                ...
                DisplayValueAttribute[] attrs = fi.GetCustomAttributes(typeof(DisplayValueAttribute), false) as DisplayValueAttribute[];
                if (attrs.Length > 0)
        ...
        /// <returns>String Value associated via a <see cref="DisplayValueAttribute"/> attribute, or null if not found.</returns>
        public static string GetStringValue(Enum value)
        {
            if ((value != null) && (value.ToString() != string.Empty) && (value.ToString() != "0"))
            {
                ...
                if (_stringValues.ContainsKey(value))
                    output = (_stringValues[value] as DisplayValueAttribute).Value;
                else
                {
                    //Look for our 'DisplayValueAttribute' in the field's custom attributes
